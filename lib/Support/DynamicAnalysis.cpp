  ...
  for (unsigned i = 0; i < nExecutionUnits + nAGUs + nPorts + nBuffers; i++)
  {
      ...
      if (i < nArithmeticExecutionUnits)
      {
        if (this->ExecutionUnitsThroughput[i] != INF)
        {
          if (this->ExecutionUnitsThroughput[i] >= 1)
          {
            ...
            roundNextMultiple (this->ExecutionUnitsThroughput[i],VectorWidth);
          }
        }
      }
      else
    ...
    if (this->ExecutionUnitsThroughput[i] > 0)
    {
      ...
      else
      {
        ...
        DEBUG (dbgs () << "AccessWidth " << AccessWidth << "\n");
        DEBUG (dbgs () << "ExecutionUnitsThroughput[i] " << this->ExecutionUnitsThroughput[i] << "\n");
        DEBUG (dbgs () << "IssueCycleGranularity " <<int (ceil(AccessWidth /
                                                               (this->ExecutionUnitsThroughput[i] *
                                                                this->ExecutionUnitsParallelIssue[i]))) << "\n");
        
        DEBUG (dbgs () << "Product " << this->ExecutionUnitsThroughput[i] *
               this->ExecutionUnitsParallelIssue[i] << "\n");
        IssueCycleGranularity =int (ceil(AccessWidth /
                                         (this->ExecutionUnitsThroughput[i] *
                                          this->ExecutionUnitsParallelIssue[i])));
